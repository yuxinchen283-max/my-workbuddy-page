...
for i, line in enumerate(lines):
    if '摄像头识别 - TensorFlow.js MoveNet' in line:
        # 找到 IIFE 的开始（上一行）
        start_line = i - 1  # 包含注释行
    if start_line >= 0 and '输入系统' in line and end_line == -1:
        end_line = i
        break

print(f'Start line: {start_line}')
print(f'End line: {end_line}')
print(f'Lines to replace: {end_line - start_line if start_line >= 0 and end_line >= 0 else 0}')
...
if start_line >= 0 and end_line >= 0:
    ...
    with open('game-v6.html', 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print('Successfully created game-v6.html')
